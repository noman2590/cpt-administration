...
function handle_custom_form_submission() {
    ...
    if (isset($_POST['handle_custom_form_nonce']) && wp_verify_nonce($_POST['handle_custom_form_nonce'], 'handle_custom_form_action')) {
        ...
        $errors = array();
        if ( empty($post_slug) ) {
            $errors['mcpt_slug_err'] = 'Post type slug is required' ?? false;
        }
        if (!empty($results) || post_type_exists($post_slug)) {
            $errors['mcpt_slug_err'] = 'Post type with this slug already exists' ?? false;
        }
        if ( empty($_POST['plural']) ) {
            $errors['mcpt_plural_err'] = 'Plural label is required' ?? false;
        }
        if ( empty($_POST['singular']) ) {
            $errors['mcpt_singular_err'] = 'Singular label is required' ?? false;
        }
        ...
        if (count($errors) > 0) {
            set_transient('mcpt_form_error', $errors, 60);
            set_transient('mcpt_form_data', $data, 60);
