...
class MCPTMainController
{
    public function __construct()
    {
        register_activation_hook( MCPT_PLUGIN_BASENAME, array( $this, 'mcpt_activation_hook' ));
        add_action('init', array($this, 'mcpt_create_custom_post_type'));
        add_action('admin_menu', array( $this, 'mcpt_admin_menu' ));
        add_action('admin_menu', array( $this, 'mcpt_register_add_new_page' ));
        add_action('admin_menu', array( $this, 'mcpt_register_edit_page' ));
        add_action('admin_enqueue_scripts', array( $this, 'mcpt_enqueue_admin_scripts'));
    }

    public function mcpt_activation_hook () {
        global $wpdb;

        $charset_collate = $wpdb->get_charset_collate();
        $table_name = $wpdb->prefix . 'mcpt_post_types';
    ...
    public static function mcpt_set_query_vars ( $args )
    {
        global $wp_query;
    ...
    public function mcpt_admin_menu() {
        add_menu_page(
            ...
            'manage_options',
            'cpt-list',
            'MCPTListController::index',
            'dashicons-editor-table',
        );
    }

    function mcpt_register_add_new_page() {
        add_submenu_page(
            'cpt-list',
            ...
            'manage_options',
            'add-new-cpt',
            'MCPTListController::mcpt_add_new_cpt'
        );
    }

    function mcpt_register_edit_page () {
        add_submenu_page(
            'cpt-list',
            ...
            'manage_options',
            'edit-cpt',
            'MCPTListController::mcpt_edit_cpt'
        );
    }

    function mcpt_enqueue_admin_scripts() {
        wp_enqueue_style( 'mcpt-style', MCPT_PLUGIN_URL . '/lib/assets/css/style.css' );
        wp_enqueue_script( 'mcpt-js', MCPT_PLUGIN_URL . '/lib/assets/js/main.js' );
        wp_enqueue_style( 'dashicons-css', MCPT_PLUGIN_URL . '/lib/assets/css/dashicons-picker.css' );
        wp_enqueue_script( 'dashicons-picker-js', MCPT_PLUGIN_URL . '/lib/assets/js/dashicons-picker.js' );
    }

    function mcpt_create_custom_post_type() {
